...
- [ ] **NbaPropLab SSR deploy fix** (HIGH leverage, task queued in NBAv3-fresh) — Angular SSR is fully built but NOT deployed: the Dockerfile ships only the static `browser/` build behind nginx, which falls back to `index.csr.html` (blank shell) for every route. Googlebot gets the same empty 18KB page for all **1,442 player pages + landing pages** → ranks only for brand "proplab". Fix = run the built Node SSR server + proxy nginx to it. Unlocks the whole 1,528-URL sitemap.
- [ ] **FutPicks SEO render fix** (task queued in Furbov2) — same class as nbaproplab: futpicks.com serves Googlebot a blank 22KB CSR shell for all 24 sitemap routes (incl. money pages /soccer-predictions, /best-soccer-picks-today, /value-betting). Angular `outputMode: "static"` but routes aren't prerendered. Fix = prerender stable landing pages + SSR/prerender the data routes with per-route meta. (8 impressions / 0 keywords today.)
